...
<script>
    layui.use(['table', 'form', 'layer','laydate','util'], function(){
        ...
        table.render({
            ...
            ,url: '/cure/cureSelectAll'
            ,cols: [[
                {type:'checkbox', fixed: 'left'}
                ,{field:'cureNo', width:100, title: '流水号', sort: true}
                ...
                ,{field:'cureTime', width:80, title: '救治日期'}
                // ,{field:'cureTime', title: '救治日期', templet: '<div>{{util.toDateString(cureTime)}}</div>'}
                ,{field:'cureContent', width:80, title: '救治描述'}
        ...
        table.on('tool(test-table-operate)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
                layer.confirm('确定删除该救助记录？', {icon: 3, title:'提示', offset: '50px'}, function(index){
                    $.ajax({
                        url:'/cure/cureDeleteByNo?cureNo=' + data.cureNo,
                        dataType:"JSON",
